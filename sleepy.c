...
MODULE_AUTHOR("Eugene A. Shatokhin, John Regehr, Elliot Hatch");
MODULE_LICENSE("GPL");
...
static int sleeping_counts[SLEEPY_NDEVICES];
static int wake_flags[SLEEPY_NDEVICES];
/* ================================================================ */
	    ...
	    loff_t *f_pos)
{
  ...
  /* wake up all the proccesses waiting on this device */
  if(sleeping_counts[minor] > 0) {
      wake_flags[minor] = 1;
	     ...
	     loff_t *f_pos)
{
  ...
  if(count != 4) {
      ...
      /* copy 4 bytes */
      if(copy_from_user(&sleep_time, buf, count) != 0) {
          retval = -EFAULT;
      }
      else {
          if(sleep_time > 0) {
              sleeping_counts[minor]++;
              mutex_unlock(&dev->sleepy_mutex); /* is this a race condition? does it matter? */
              remainingJiffies = wait_event_interruptible_timeout(wait_queue, wake_flags[minor] != 0, HZ*sleep_time);
			  if (mutex_lock_killable(&dev->sleepy_mutex)) {
				  /* failed. just mutate the stuff anyway and return early */
				  sleeping_counts[minor]--;
				  if(sleeping_counts[minor] == 0) {
					  wake_flags[minor] = 0;
				  }
...
sleepy_init_module(void)
{
  ...
  /* initialize sleep flags and lock counters */
  for(i = 0; i < SLEEPY_NDEVICES; ++i) {
      sleeping_counts[i] = 0;
      wake_flags[i] = 0;
  }
